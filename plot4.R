...
# plot4 is produced on the screen device
...
asseY4 <- as.numeric(as.character(myData$Voltage))

asseY5 <- as.numeric(as.character(myData$Global_reactive_power))

par(mfrow = c(2,2))

plot(asseX, asseY, type = "l", xlab = "datetime", ylab = "Global Active Power (kilowatts)")

plot(asseX, asseY4, type = "l", xlab = "datetime", ylab = "Voltage")
